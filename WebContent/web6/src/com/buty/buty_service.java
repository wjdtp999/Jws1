...
public class buty_service {
	...
	public void all_data(HttpServletRequest request) {
		...
		exchange_dao ed=new exchange_dao();
		int total=ed.select_count("buty");
		
		ArrayList<exchange> list= ed.all_select("buty",page_num==1? 1:(page_num-1)*10,10);
	...
	public void save(HttpServletRequest request) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("뷰티 이미지 오류");
		}
		String filename = fname[0]+"-"+fname[1]+"-"+fname[2];
		exchange_dao ed = new exchange_dao();
		ed.board_save("buty",writer,title,cont,filename);
	}
}
